...
{
    public class CadastroMateriaViewModel : INotifyPropertyChanged
    {
        public CadastroMateriaViewModel()
        {
            CriarEnabled = true;
        ...
        public void ExecutarCriar(object obj)
        {
            ...
            {
                MessageBox.Show("Insira um nome para a nova matéria.");
                return;
            }
            ...
            var task = new Task(() =>
            {
                ...
                var model = new CadastrarMateriasModel();
                model.Execute(new CadastrarMateriasRequest()
                {
                    Nome = text
                });

                if (model.Response.Status == ExecutionStatus.Success) MessageBox.Show("Matéria criada com sucesso!");
                else MessageBox.Show(string.Concat("Erro ao criar a matéria:\n", model.Response.ErrorMessage));
